...
| `retrieval_ready` | boolean | 是 | 是否满足当前知识库运行配置的 QA 检索门槛 |
...
- 设置为 active 前，`parse_status` 必须为 `success`，且当前 active `ConfigRevision` 要求的检索副本状态必须为 `success` 或 `not_required`。
- `retrieval_ready = true` 表示该版本已满足当前运行配置的最小检索要求；例如未启用 Sparse 时，`sparse_index_status = not_required` 不阻塞激活。
...
OpenSearch 是可选文本检索副本，仅在启用 Sparse 或 Hybrid 检索时需要，不作为正文真值来源。
