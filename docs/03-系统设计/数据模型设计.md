...
| Neo4j | 图结构主存，启用图索引能力时维护 | Entity、Relation、Community、ChunkRef | 图增强结果必须通过 `chunk_id` 回落到 PostgreSQL Chunk |
...
erDiagram
    ...
    knowledge_bases ||--o{ document_kb_bindings : receives
    documents ||--o{ document_kb_bindings : binds
    documents ||--o{ document_versions : versions
    document_versions ||--o{ library_parse_jobs : extracted_by
    ...
    knowledge_bases ||--o{ rag_apps : publishes
    config_revisions ||--o{ rag_apps : defaults
    rag_apps ||--o{ rag_app_api_keys : authenticates
    rag_apps ||--o{ app_conversations : owns
    app_conversations ||--o{ app_messages : contains
    app_messages ||--o{ app_invocations : invokes
    qa_runs ||--o{ app_messages : answers
    qa_runs ||--o{ app_invocations : traces
```
...
- `RagApp` 绑定知识库和默认配置版本，只作为外部调用入口，不复制 Pipeline 真值。
- `AppMessage` 与 `AppInvocation` 必须能关联 QARun，确保外部会话进入内部治理闭环。
...
| `owner_id` | uuid | 是 | 文档库归属用户，引用 `users.user_id` |
...
- `owner_id` 是文档库最小访问边界；文档绑定到知识库前不生成知识库级 Chunk 或检索副本。
- 文档逻辑删除后，相关绑定、Chunk 和检索副本通过异步任务标记失效或清理。
- 历史 `kb_id` 模式数据迁移时应生成 `document_kb_bindings`，避免同一归属事实长期由两个字段维护。
...
| `chunk_count` | integer | 否 | 知识库入库版本的 Chunk 数量；文档库源文件版本为空 |
...
- 文档库源文件版本只保存解析结果和预览，不维护 `chunk_count`；真实分块数量由知识库入库后的 `chunks`、`chunk_revisions` 和 `document_kb_bindings` 维护。
...
| `binding_id` | uuid | 否 | 引用 `document_kb_bindings.binding_id`，用于文档库绑定链路追踪 |
...
#### 4.4.5 `document_kb_bindings`

用于表达个人文档库文档与知识库之间的多对多绑定关系。绑定成功后才触发知识库级切块、Embedding、Sparse 索引和图构建。
...
| `document_id` | uuid | 是 | 引用 `documents.document_id` |
| `kb_id` | uuid | 是 | 引用 `knowledge_bases.kb_id` |
| `version_id` | uuid | 是 | 本次绑定使用的文档版本 |
| `chunk_size` | integer | 是 | 绑定级切块大小 |
| `chunk_overlap` | integer | 是 | 绑定级切块重叠 |
| `status` | varchar(16) | 是 | `pending` / `processing` / `active` / `failed` / `disabled` |
| `chunk_count` | integer | 是 | 当前绑定产生的 Chunk 数 |
| `error_code` | varchar(64) | 否 | 失败错误码 |
| `error_message` | text | 否 | 失败原因 |
| `metadata` | jsonb | 否 | 扩展属性 |
| `created_at` / `created_by` | timestamptz / uuid | 是/否 | 创建审计 |
...
- `version_id` 必须属于同一 `document_id`。
- 同一 `document_id + kb_id + version_id` 未停用时唯一。
- 解绑不物理删除绑定，改为 `disabled` 并通过异步作业清理或标记检索副本失效。

### 4.5 Ingest 与索引同步

#### 4.5.1 `library_parse_jobs`

用于记录文档库上传后的文本提取、预览数据生成和文档库级重新解析作业。该表不替代知识库绑定后的 `ingest_jobs`。

| 字段名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| `job_id` | uuid | 是 | 主键 |
| `document_id` | uuid | 是 | 文档 ID |
| `version_id` | uuid | 是 | 文档版本 ID |
| `job_type` | varchar(32) | 是 | `extract_text` / `generate_preview` / `reparse_library` |
...
| `error_message` | text | 否 | 错误详情 |
| `created_at` / `started_at` / `finished_at` | timestamptz | 是/否 | 作业时间 |
...
- 启用图索引能力时，Neo4j 是图结构主存，PostgreSQL 仅保存快照和回溯摘要。
...
| `app_id` | uuid | 是 | 主键 |
| `kb_id` | uuid | 是 | 绑定知识库 |
| `default_config_revision_id` | uuid | 否 | 默认配置版本；为空时使用 KB active revision |
| `name` | varchar(128) | 是 | 应用名称 |
| `description` | text | 否 | 应用说明 |
| `status` | varchar(16) | 是 | `active` / `disabled` / `archived` |
| `output_policy` | jsonb | 是 | 外部响应策略，如 citation、metadata 输出 |
...
- `kb_id` 必须指向未删除知识库。
- `default_config_revision_id` 如存在，必须属于同一知识库且可运行。
- RAG App 不复制 `pipeline_definition`，运行时仍读取 ConfigRevision。

#### 4.10.2 `rag_app_api_keys`

用于保存应用级调用凭据摘要。

| 字段名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| `api_key_id` | uuid | 是 | 主键 |
| `app_id` | uuid | 是 | RAG App |
| `key_hash` | varchar(128) | 是 | API Key hash |
| `key_prefix` | varchar(16) | 是 | 明文前缀摘要，用于管理端识别 |
| `status` | varchar(16) | 是 | `active`；旧禁用记录可能保留 `revoked` |
| `expires_at` | timestamptz | 否 | 过期时间 |
| `last_used_at` | timestamptz | 否 | 最近使用时间 |
| `created_at` / `created_by` | timestamptz / uuid | 是/否 | 创建审计 |
| `revoked_at` / `revoked_by` | timestamptz / uuid | 否 | 兼容旧禁用审计；新增删除不写入 |

约束：

- API Key 明文只在生成时返回一次，不得落库。
- `key_hash` 唯一。
- 删除 API Key 时物理删除本表记录；历史调用记录保留，但解除 `app_invocations.api_key_id` 关联。

#### 4.10.3 `app_conversations`

用于记录外部应用会话。

| 字段名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| `conversation_id` | uuid | 是 | 主键 |
| `app_id` | uuid | 是 | RAG App |
| `end_user_id` | varchar(128) | 否 | 外部终端用户标识 |
| `status` | varchar(16) | 是 | `active` / `archived` |
| `metadata` | jsonb | 是 | 来源、渠道等摘要 |
| `created_at` | timestamptz | 是 | 创建时间 |
| `updated_at` | timestamptz | 是 | 更新时间 |
...
| 字段名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| `message_id` | uuid | 是 | 主键 |
| `conversation_id` | uuid | 是 | 外部会话 |
| `role` | varchar(16) | 是 | `user` / `assistant` |
| `content` | text | 是 | 消息正文 |
| `qa_run_id` | uuid | 否 | 助手回答关联 QARun |
| `status` | varchar(16) | 是 | `running` / `success` / `failed` |
| `metadata` | jsonb | 是 | citations、usage、安全摘要等；员工培训助手答题结果写入 `trainingResult` |
| `created_at` | timestamptz | 是 | 创建时间 |

约束：

- 助手消息成功时必须关联 `qa_run_id`。
- 外部消息记录不替代 QARun 详情，完整 Trace 仍保存在 QARun 相关表。
- `metadata.trainingStructuredRun` 保存培训讲解或测验生成的结构化输出快照；`metadata.trainingResult` 保存答题得分、通过状态、逐题结果、错题解释和关联测验消息，用于应用级培训报告聚合。

#### 4.10.5 `app_invocations`

用于记录 App Runtime 单次调用审计。

| 字段名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| `invocation_id` | uuid | 是 | 主键 |
| `app_id` | uuid | 是 | RAG App |
| `api_key_id` | uuid | 否 | 使用的 Key 摘要；Key 删除后置空 |
| `conversation_id` | uuid | 否 | 外部会话 |
| `message_id` | uuid | 否 | 助手消息 |
| `qa_run_id` | uuid | 否 | 关联 QARun |
| `status` | varchar(16) | 是 | `running` / `success` / `failed` |
| `error_code` | varchar(64) | 否 | 失败错误码 |
| `latency_ms` | integer | 否 | 调用耗时 |
| `request_summary` | jsonb | 是 | 请求摘要，不保存密钥明文 |
| `response_summary` | jsonb | 是 | 响应摘要 |
| `created_at` | timestamptz | 是 | 创建时间 |

说明：

- Invocation 用于外部调用审计和排障，不替代 `audit_logs` 的高风险操作审计。
- 有效 App/Key 请求进入 Runtime 后先写入 `running` 记录，调用完成后更新为 `success` 或 `failed`；超过并发上限的请求直接写 failed 记录，不进入队列。
- 成功调用必须能通过 `qa_run_id` 追溯到 ConfigRevision、Evidence、Citation 和 Trace。
...
Neo4j 在知识库或部署启用图索引能力时保存图结构主数据，但不能直接提供最终 Citation。
...
| `document_kb_bindings.status` | `pending`、`processing`、`active`、`failed`、`disabled` | CHECK |
...
| `library_parse_jobs.status` | `queued`、`running`、`success`、`failed`、`cancelled` | CHECK |
...
| `rag_apps.status` | `active`、`disabled`、`archived` | CHECK |
| `rag_app_api_keys.status` | `active`；旧禁用记录可能保留 `revoked` | CHECK |
| `app_conversations.status` | `active`、`archived` | CHECK |
| `app_messages.status` | `success`、`failed` | CHECK |
| `app_invocations.status` | `running`、`success`、`failed` | CHECK |
...
- `documents(owner_id, status, updated_at)`
- `document_kb_bindings(kb_id, status, updated_at)`
- `document_kb_bindings(document_id, status, updated_at)`
- `library_parse_jobs(document_id, status, created_at)`
...
- `rag_apps(kb_id, status, created_at)`
- `app_conversations(app_id, end_user_id, updated_at)`
- `app_messages(conversation_id, created_at)`
- `app_invocations(app_id, status, created_at)`
...
Sprint 54 不新增图片专用业务表，图片 Provider 硬化继续复用现有 JSONB metadata：

| 对象 | 字段 | 说明 |
| --- | --- | --- |
| `parse_revisions.parse_options` | `sourceModality`、`provider`、`model`、`maxImageSide`、`authHeader` | 记录视觉解析策略，不记录密钥 |
| `document_versions.metadata` | `sourceModality`、`visionTextProvider`、`visionModel`、`sourceMimeType`、`sourceFileSize`、`imageTokens` | 文档版本级视觉抽取摘要 |
| `chunks.metadata` | `sourceModality`、`region`、`visionProvider`、`visionModel`、`sourceMimeType`、`sourceFileSize`、`imageTokens` | 图片 Chunk 的检索和 Citation 来源摘要 |
| `qa_run_citations.location_snapshot` | `sourceModality`、`sourceFileId`、`region`、`imageWidth`、`imageHeight` | 图片 Citation 定位，第一阶段 `region=full` |

图片原件仍保存在对象存储并由 `stored_files` 回溯；PostgreSQL 只保存视觉文本、Chunk 真值和安全摘要，不保存 base64。
...
| 应用运行时 | `rag_apps`、`rag_app_api_keys`、`app_conversations`、`app_messages`、`app_invocations` | App 和 Key 可禁用；会话、消息和调用记录按保留策略归档 |
...
- 业务实体覆盖：User、Group、KB、Document、Version、Chunk、Ingest、ConfigRevision、QARun、Evidence、Citation、Trace、GraphSnapshot、EvaluationSample、RagApp、AppConversation、AppMessage、AppInvocation 均有模型承接。
