...
- 设置为 active 前，`parse_status` 必须为 `success`，且知识库索引能力要求的检索副本状态必须为 `success` 或 `not_required`。
- `retrieval_ready = true` 表示该版本已满足知识库索引能力配置要求；例如知识库未启用 Sparse 索引能力时，`sparse_index_status = not_required` 不阻塞激活。
