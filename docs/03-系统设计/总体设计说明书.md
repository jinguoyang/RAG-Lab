...
7. 若启用 Sparse 或 Hybrid 检索，则写入 OpenSearch 副本。
8. 若启用图检索增强，则执行图抽取并写入 Neo4j，同时记录 `GraphSnapshot` 元数据。
9. Worker 更新各副本就绪状态，满足当前配置的最小检索要求后将版本标记为 `retrieval_ready`。
...
- OpenSearch 和 Neo4j 均为按需能力；未启用对应能力时，其副本状态应为 `not_required`，不得阻塞文档版本激活。
