...
- 将稳定的知识库与配置版本发布为 RAG App，让外部 Web 应用通过受控对话接口复用平台治理能力。

### 3.3 非目标

- 不建设公网匿名聊天产品；RAG App 运行时只面向受控外部 Web 应用调用。
- 不在本期建设复杂审批流、计费中心、BI 统计中心。
- 不在本期建设多租户 SaaS 隔离体系。
- 不建设 Dify 式自由工作流、插件市场或通用 Agent 编排平台。
...
- RAG App 创建、配置绑定、App API Key 管理和外部对话运行时
- 外部会话、消息、调用审计和质量反馈回流
...
- QA 调试以单次实验和回放分析为主；RAG App 运行时优先满足受控外部 Web 应用的查询对话，不追求公网匿名海量聊天吞吐。
- 文档正文真值必须保存在 PostgreSQL，不允许仅保存在对象存储、向量库或图数据库中。
- 图检索增强必须回落到授权 Chunk 才能进入回答上下文。
- 外部应用调用必须能追溯到 App、Conversation、Message、Invocation、QARun 和 ConfigRevision。
...
flowchart LR
    ...
    FE --> API["后端 API 服务"]
    EXT["外部 Web 应用"] --> RT["RAG App Runtime API"]
    RT --> API
...
- 文档解析与切块
- 图片文件通过视觉模型抽取文本后切块，第一阶段默认对齐小米 `mimo-v2.5` 图片理解接口
- Embedding 生成与向量写入
- 按知识库索引能力写入 OpenSearch 文本索引
- 按知识库索引能力抽取图结构并写入 Neo4j
- 批量重建、重试和补偿任务
...
| RAG 应用封装模块 | 将 KB 与 ConfigRevision 发布为可外部调用的受控 RAG 应用，并承载知识问答助手和员工培训助手两个内置场景 | RagApp、AppApiKey、AppConversation、AppMessage、AppInvocation | RAG Apps、App Runtime、培训报告 |
...
| RagApp | 对外发布的 RAG 应用 | 绑定 KnowledgeBase 与默认 ConfigRevision |
| RagAppApiKey | 应用级调用凭据 | 只保存 key hash，归属于 RagApp |
| AppConversation | 外部应用会话 | 归属于 RagApp，包含多条 AppMessage |
| AppMessage | 外部用户消息或助手回答 | 助手回答关联 QARun |
| AppInvocation | 外部运行时调用审计 | 关联 RagApp、AppMessage 和 QARun |
...
- `RagApp` 只封装对外调用入口，不替代知识库、配置版本或 QARun。
- `AppMessage` 的助手回答必须能关联到一次 QARun，外部会话不作为 RAG 执行真值。
...
9. Worker 更新各副本就绪状态，满足知识库中阻塞激活的索引能力要求后将版本标记为 `retrieval_ready`。
...
- 是否阻塞文档版本激活由知识库索引能力中的 `requiredForActivation` 策略决定；图索引默认建议不阻塞 Dense / Sparse 基础 QA。
...
1. 知识库负责人创建 RAG App，绑定 `kbId` 和默认 `configRevisionId`；未绑定时默认使用 KB active revision。
2. 管理端生成 App API Key，服务端只保存 key hash。
3. 外部 Web 应用调用 App Runtime API，并通过 App API Key 鉴权。
4. 后端校验 App、Key、知识库和配置版本状态。
5. 后端创建或读取 `AppConversation`，记录用户 `AppMessage`。
6. 运行时服务将外部请求转换为受控 QARun，复用现有 QA 编排、权限裁剪、Evidence、Citation 和 Trace。
7. QARun 完成后写入助手 `AppMessage` 和 `AppInvocation`。
8. 外部接口返回 answer、citations、conversationId、messageId、runId、usage 和安全 metadata。
9. 外部反馈可关联 messageId 或 runId，回流为 QARun 反馈或 EvaluationSample。

设计要点：

- App Runtime 不接受外部传入的主体级 Chunk 过滤条件；授权和过滤仍由后端根据 App 绑定和平台策略生成。
- 外部响应不暴露完整内部 Trace、未授权候选正文、Provider 密钥或系统提示词。
- `runId` 是外部结果进入内部治理、回放、对比和评估闭环的稳定锚点。
...
| RAG App 管理 | RagApp | 需具备应用管理权限或知识库管理权限 |
| App Runtime 调用 | RagAppApiKey | 需使用有效、未禁用的 App API Key |
| 外部回答输出 | AppMessage / QARun | 只能输出授权 Evidence 与 Citation 的安全摘要 |
