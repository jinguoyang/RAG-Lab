...
flowchart LR
    ...
    MQ --> EMB["Embedding / 索引 Worker"]
    MQ --> GRAPH["图构建 Worker"]

    EMB --> VDB["Milvus / pgvector"]
    EMB --> OS["OpenSearch（可选）"]
    GRAPH --> NEO["Neo4j"]

    QA --> VDB
    QA --> OS
    QA --> NEO
    QA --> LLM["LLM / Rerank / Rewrite 服务"]
    QA --> PG
```
...
- 后端：Python FastAPI
- 任务框架：Celery + Redis，或等价的异步任务框架
- 数据库：PostgreSQL
- 对象存储：MinIO 或兼容 S3 的对象存储
- 向量检索：
  - 推荐方案：Milvus
  - 简化方案：PostgreSQL `pgvector`
- 关键词检索：OpenSearch（按需启用）
- 图数据库：Neo4j
...
1. 用户在知识库中上传文档。
2. 后端创建 `Document` 和新的 `DocumentVersion` 记录。
3. 系统创建 `IngestJob`，状态初始化为 `queued`。
4. Worker 执行文档解析、切块、Chunk 写入 PostgreSQL。
5. Worker 执行 Embedding 计算并写入向量检索层。
6. 若启用 Sparse 检索，则写入 OpenSearch 副本。
7. Worker 执行图抽取并写入 Neo4j，同时记录 `GraphSnapshot` 元数据。
8. 作业完成后更新 `IngestJob` 状态，并在前端展示结果。
...
1. 用户从模板或当前 Revision 编辑配置。
2. 保存操作生成新的 `ConfigRevision`。
3. 新 Revision 默认处于“已保存未生效”状态。
4. 用户确认切换后，将目标 Revision 标记为 active。
5. 后续 QA 调试默认基于当前 active Revision 运行。
...
3. 执行 Query Rewrite。
4. 按配置执行 Dense、可选 Sparse、可选 Graph 检索。
5. 对多路候选进行去重、融合和 Rerank。
6. 对最终候选执行权限过滤。
7. 组织上下文并调用 LLM 生成答案。
8. 产出 `Evidence / Citation / Trace / Metrics`。
9. 持久化 `QARun` 及明细，供历史查询和回放。
