...
6. 后端将目标 Revision 设为 active，并记录审计日志。
...
3. 锁定本次使用的 active `ConfigRevision`。
4. 合并临时覆盖参数，但不得修改 active revision。
5. 执行 Query Rewrite。
6. 执行 Dense、Sparse、Graph 检索，允许部分通道失败后降级。
7. 对候选进行去重、融合、Rerank。
8. 执行 Permission Filter，裁剪未授权候选。
9. 构造上下文并调用 LLM 生成。
10. 构建 Citation 和 Evidence。
11. 持久化 Trace、Metrics、候选摘要和最终结果。
