...
| 对象 | 说明 | 关键字段 |
| --- | --- | --- |
| `KnowledgeBase` | 知识库工作空间 | kbId、name、description、ownerId、defaultSecurityLevel、sparseIndexEnabled、graphIndexEnabled、requiredForActivation、status |
...
1. 平台管理员或具备权限的用户提交知识库名称、描述、负责人、默认密级和索引能力开关。
2. 后端校验名称、权限、默认密级和当前部署可用索引能力。
...
- 功能说明：管理个人文档库上传、文本提取、在线预览、文档与知识库绑定、Chunk、重解析、Ingest 作业、Dense/Sparse/Graph 副本构建。
- 关键页面：P06 文档中心、P07 文档详情、P15 我的文档库、P16 文档库详情、P05 知识库概览中的最近作业区。
- 关键接口：文档库上传、文档库列表、文档库详情、文档使用情况、文档绑定知识库、知识库文档列表、版本列表、Chunk 查询、Chunk 详情、IngestJob 查询、失败重试。
...
| 对象 | 说明 | 关键字段 |
| --- | --- | --- |
| `Document` | 个人文档库文档主对象 | docId、ownerId、name、securityLevel、status、activeVersionId |
| `DocumentVersion` | 文档版本与文本提取结果 | versionId、docId、versionNo、sourceFileId、parseStatus、isActive |
| `DocumentKbBinding` | 文档与知识库绑定 | bindingId、docId、kbId、versionId、chunkSize、chunkOverlap、status |
| `Chunk` | 文档切块真值 | chunkId、versionId、pageNo、section、content、tokenCount、metadata |
| `LibraryParseJob` | 文档库文本提取作业 | jobId、docId、versionId、type、status、progress、errorMessage |
| `IngestJob` | 知识库入库作业 | jobId、kbId、docId、versionId、type、status、progress、errorMessage |
| `GraphSnapshot` | 图构建快照 | graphSnapshotId、kbId、versionScope、jobId、status |

#### 3.3.2 上传与入库流程

1. 用户在 P15 上传文档到个人文档库。
2. 后端创建 `Document` 和 `DocumentVersion`，并保存原始文件到对象存储 `users/{user_id}/library/{doc_id}/...`。
3. 创建 `LibraryParseJob`，状态为 `queued`，用于文本提取和预览数据生成。
4. Worker 提取文本并更新 `DocumentVersion.parse_status`，成功后 P16 可展示预览；此阶段不生成知识库级 Chunk。
5. 用户在 P06 或 P16 将文档绑定到知识库，后端创建 `DocumentKbBinding` 并校验文档 owner 与知识库权限。
6. 绑定后创建 `IngestJob`，根据绑定参数生成 Chunk，并将 Chunk 真值写入 PostgreSQL。
7. Worker 生成 embedding，写入 Dense Provider 对应副本。
8. 若知识库启用 Sparse 索引能力，则写入 Sparse Provider 对应副本。
9. 若知识库启用图索引能力，则抽取实体、关系、社区摘要并写入 Graph Provider。
10. 作业成功后更新绑定和版本状态；失败时保留错误信息和重试入口。

索引能力与运行策略边界：

- Sparse 索引能力和图索引能力属于知识库或部署层配置，决定文档处理阶段是否维护 OpenSearch、Neo4j 副本。
- Config Revision 和 QARun 中的 Dense / Sparse / Graph 开关属于运行策略，只决定本次 QA 是否调用对应检索 Provider。
- Hybrid 是 QA 运行阶段的候选融合策略，不是独立索引副本，也不决定文档入库时是否写 OpenSearch。

#### 3.3.3 Active Version 切换

- 同一 `Document` 任一时刻只允许一个 active version。
- 切换 active version 前必须校验用户权限，并提示会影响后续检索和 QA。
- 目标版本的解析状态必须为成功，并且知识库中标记为 `requiredForActivation` 的 Dense / Sparse / Graph 副本必须已完成或标记为 `not_required`。
...
#### 3.3.5 图片解析与 mimo-v2.5 Provider 硬化

- 独立图片文件继续走文档库和知识库绑定流程，支持 `jpg/jpeg/png/gif/webp/bmp`。
- 图片解析阶段调用 `VisionTextProvider`，默认按小米 `mimo-v2.5` 图片理解接口生成视觉文本 Markdown，再进入现有 ParseRevision、Chunk、Dense/Sparse/Graph、QA、Evidence 和 Citation 链路。
- HTTP Provider 使用 OpenAI-compatible `chat.completions` 图片消息，`image_url.url` 必须使用实际 MIME 的 `data:{mime};base64,...`；默认鉴权头为 `api-key`，可按部署需要配置为 Bearer 兼容模式。
- base64 编码后超过 50 MB 的图片在 Provider 调用前拒绝，避免把超限图片发送到外部模型服务。
- Provider usage 中的 `image_tokens`、模型名、MIME、文件大小等只作为安全摘要写入 metadata，不保存 API Key、base64 或原图二进制。
- `docs/examples/1I3A6520-opq3542107848.jpg` 和 `docs/examples/oxlndt5t1zr31.jpg` 作为 Sprint 54 验收样本，需能通过文本问答分别召回乔迁照片和猫咪照片。
...
8. 如需复用历史 Revision，用户在 P08 Revision 历史中选择“复制为草稿”；后端基于源 Revision 生成新的 draft，后续仍需校验、保存和激活。

#### 3.4.5 设计约束

- P08 不执行单次 QA，不展示单次召回 Chunk。
- Revision 激活必须可审计，至少记录操作者、时间、旧 revision、新 revision。
- archived Revision 不允许原地重新激活；复用历史配置必须先复制为新草稿，避免修改历史审计语义。
...
### 3.9 RAG 应用封装与外部运行时模块

- 功能说明：将已治理的知识库和 ConfigRevision 发布为可被外部 Web 应用调用的 RAG App，并保留会话、消息、调用审计和 QARun 回溯关系。
- 关键页面：P13 RAG 应用管理页。
- 关键接口：RAG App 管理、App API Key 生成/删除/轮换、App Runtime 对话、结构化运行、培训答题提交、培训报告、外部反馈回流。
- 关键状态：app active、disabled、archived；key active；conversation active、archived；message success、failed。

#### 3.9.1 对象边界

| 对象 | 说明 | 关键字段 |
| --- | --- | --- |
| `RagApp` | 外部可调用的应用封装 | appId、kbId、defaultConfigRevisionId、status、outputPolicy |
| `RagAppApiKey` | 应用调用凭据 | apiKeyId、appId、keyHash、status、expiresAt、lastUsedAt |
| `AppConversation` | 外部对话上下文 | conversationId、appId、endUserId、status |
| `AppMessage` | 用户消息或助手回答 | messageId、conversationId、role、content、qaRunId、status |
| `AppInvocation` | 单次运行时调用审计 | invocationId、appId、messageId、qaRunId、status、latencyMs、errorCode |

设计约束：

- `RagApp` 只绑定知识库和配置版本，不复制 `pipelineDefinition`。
- `AppConversation` 和 `AppMessage` 是外部应用体验层记录，不替代 QARun 历史。
- 助手消息必须关联 QARun；若运行失败，也要记录失败状态和错误码，便于排障。
- API Key 明文只在生成时返回，落库只保存 hash 和安全摘要；删除 Key 时物理删除凭据行，调用审计保留但解除 Key 关联。
- 员工培训助手不新增 LMS 表；培训讲解、测验和答题结果写入 `AppMessage.metadata`，应用级培训报告按 App 会话消息聚合。

#### 3.9.2 App Runtime 执行流程

1. 外部 Web 应用提交 `query`、可选 `conversationId`、`endUserId`、`inputs` 和 `responseMode`。
2. 运行时鉴权 App API Key，解析当前 `appId`。
3. 后端校验 App、知识库、默认配置版本和运行策略。
4. 创建或读取 Conversation，并写入用户 Message。
5. 将请求转换为内部 QARun：`query` 进入 QARun，App 信息写入 override 或 metadata 快照。
6. 复用现有 QA 编排执行 Query Rewrite、Dense/Sparse/Graph、Fusion/Rerank、Permission Filter、Generation 和 Citation。
7. 读取 QARun 终态结果，按 App 输出策略生成外部响应。
8. 写入助手 Message 与 Invocation 审计。
9. 返回 answer、citations、conversationId、messageId、runId、usage 和安全 metadata。

#### 3.9.3 输出策略

- 第一版默认返回 answer、Citation 摘要、runId、usage 和精简 metadata。
- 外部响应不得返回完整 Trace、未授权候选正文、Provider 密钥、系统提示词或内部配置明细。
- Citation 必须来自授权 Evidence；如果没有授权 Evidence，应返回稳定错误或明确的“无可用证据”回答策略。
- `responseMode = blocking` 为首版必选能力；SSE streaming 后续作为兼容扩展。

#### 3.9.4 治理回流

- 外部反馈可通过 `messageId` 或 `runId` 关联到 QARun。
- 有效反馈可更新 QARun 反馈状态，或沉淀为 EvaluationSample。
- 应用维度的失败率、无证据率、引用错误率和延迟统计后续可进入 P13 或观测模块。
- 反馈回流必须遵守现有 QA 历史和评估权限，不允许外部调用方读取超出 App 绑定范围的历史事实。
...
| P06 | 文档中心 | 上传文档、从文档库添加、批量重解析、筛选文档、查看作业 | Documents、DocumentKbBindings、IngestJobs | 绑定到知识库后生成知识库级 Chunk |
...
| P15 | 我的文档库 | 上传个人文档、搜索、批量操作、进入详情 | LibraryDocuments、LibraryParseJobs | 只展示当前用户自己的文档 |
| P16 | 文档库详情 | 在线预览、查看元数据、查看使用情况、绑定知识库 | LibraryDocument、DocumentKbBindings | 预览不替代 PostgreSQL 真值 |
| P13 | RAG 应用管理 | 创建应用、绑定配置、生成/删除 API Key、查看调用摘要 | RAG Apps、App Runtime | 外部调用必须回溯 QARun |
...
| `library.document.update` | LibraryDocument | 修改自己的个人文档 |
