...
- 关键页面：P13 RAG 应用管理页。
- 关键接口：RAG App 管理、App API Key 生成/禁用/轮换、App Runtime 对话、外部反馈回流。
- 关键状态：app active、disabled、archived；key active、revoked；conversation active、archived；message success、failed。
...
- API Key 明文只在生成时返回，落库只保存 hash 和安全摘要。
...
| P13 | RAG 应用管理 | 创建应用、绑定配置、生成/禁用 API Key、查看调用摘要 | RAG Apps、App Runtime | 外部调用必须回溯 QARun |
