...
| S1-004 | B-004 | 实现 `/health` 健康检查接口 | P0 | 0.5d |  | Done |
| S1-005 | B-005 | 补充前后端本地启动说明和最小验证命令 | P0 | 0.5d |  | Done |
...
- 决策：健康检查挂载在 `GET /api/v1/health`，纳入当前 API 版本前缀。
- 范围：只返回服务进程级状态、应用名称、版本和运行环境，不探测数据库、对象存储、检索组件或模型服务。
- 已完成：新增 `backend/app/api/routes/health.py` 和 `backend/app/schemas/health.py`，并在 API 路由聚合器中注册。
- 后续：如需依赖级健康检查，应另建 readiness 或 dependency diagnostics 接口，避免基础探活被外部组件状态拖慢。

### S1-005 本地启动与验证说明

- 决策：根 README 提供跨工程启动和最小验证命令，前端/后端 README 提供各自工程内的验证说明。
- 范围：只补充文档，不新增脚本或功能代码。
- 已完成：补充前端 `npm run build`、后端 `compileall` 和 `/api/v1/health` TestClient 验证命令。
- 后续：新增 lint、test、数据库迁移后，应同步扩展最小验证命令。
