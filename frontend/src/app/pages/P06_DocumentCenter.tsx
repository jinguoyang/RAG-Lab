...
import { Search, Upload, Download, FileWarning, RefreshCw } from "lucide-react";
...
export function DocumentCenter() {
  ...
  async function loadData(keyword = searchTerm) {
    if (!kbId) return;
    setLoading(true);
    try {
      const [documentPage, jobPage] = await Promise.all([
        fetchDocuments(kbId, keyword),
        ...
        <div className="space-y-6 min-w-0">
          <div className="flex flex-wrap items-center gap-4">
            ...
            <div className="ml-auto text-sm text-stone-gray">
              {loading ? "加载中..." : `当前结果 ${filteredRows.length} 条`}
            </div>
          </div>
                ...
                <TableBody>
                  {filteredRows.map((doc) => (
                    ...
                    >
                      ...
                      <TableCell>{doc.updatedAtLabel}</TableCell>
      ...
      <Drawer isOpen={isUploadOpen} onClose={() => setIsUploadOpen(false)} title="上传文档" width="560px">
        ...
        <DrawerSection title="提交结果">
          <p className="text-sm text-stone-gray">
            当前 Sprint 只保存文件元数据和存储引用，不写入真实 MinIO，也不执行解析 Worker。
          </p>
